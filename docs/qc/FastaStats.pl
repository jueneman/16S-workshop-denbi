...
sub printContiglengths {
    my ($h) = @_;    
    my $ph;
    foreach(sort {$a<=>$b} keys(%$h)) {foreach my $count (1..$h->{$_}) {$ph.= sprintf("%d\t%d\n",$_, $h->{$_});};}
    return $ph;
}
...
sub computeIQR {
    my ($h) = @_;
    my @ph;
    foreach(sort {$a<=>$b} keys(%$h)) {foreach my $count (1..$h->{$_}) {push(@ph, $_);};}
    my $len = scalar(@ph);
    my $q1i; my $q3i; 
    if ($len%2 == 0) {
    	$q1i = ((($len+1)/2)+0.5)/2;
    ...
    my $q3 = $ph[$q3i];
    my $IQR = $q3-$q1;
    return sprintf("Q1=%d Q3=%d IQR=%d low=%d high=%d", $q1, $q3, $IQR, ($q1- (1.5*$IQR)), ($q3+(1.5*$IQR)));
}
